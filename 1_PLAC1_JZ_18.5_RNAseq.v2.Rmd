...
ggplot(d, aes(PC1, PC2, color = condition)) +
  ...
  scale_color_manual(values = c('KO'	= '#00b462',
                                'WT' 	  = '#980c80')) +
  theme_bw() +
...
```{r vol2, fig.cap="Fig X: KO vs WT (purple/negFC = overexpressed in WT; green/posFC = overexpressed in KO)", fig.width=5, fig.height=3, warnings=FALSE, message=FALSE}
ggplotly(g)
```
...
## Enrichment tests (green/posFC = overexpressed in KO) {.tabset}
...
## Enrichment tests (purple/negFC = overexpressed in WT) {.tabset}
...
# create named vector with geneID and Log2FC
