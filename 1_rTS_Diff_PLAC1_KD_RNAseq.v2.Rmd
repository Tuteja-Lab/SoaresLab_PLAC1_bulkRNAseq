---
title: "rTS Diff PLAC1 KD Bulk RNAseq analyses"
date: "`r Sys.Date()`"
author:
  - name: Arun Seetharam
...
conda activate star
# working dir
mkdir -p /work/LAS/geetu-lab/arnstrm/rTS_Diff_PLAC1_KD
cd /work/LAS/geetu-lab/arnstrm/rTS_Diff_PLAC1_KD
mkdir -p 1_data
mkdir -p 2_fastqc
...
cd 1_data
rsync -avP /lss/folder/path/rTS_Diff_PLAC1_KD ./1_data
...
# set path
setwd("/work/LAS/geetu-lab/arnstrm/rTS_Diff_PLAC1_KD")
# load the modules
library(tidyverse)
library(DESeq2)
library(pheatmap)
...
ggplot(d, aes(PC1, PC2, color = condition)) +
  ...
  scale_color_manual(values = c('PLAC1.KD'	= '#00b462',
                                'SCR' 	  = '#980c80')) +
  theme_bw() +
...
processDE.id(res.PLCvsSCR, "PLAC1vsSCR")
```

## Volcano plots {.tabset}

### PLAC1 vs. SCR (label 1)

```{r vol1, fig.cap="Fig X: PLAC1 vs SCR (purple/negFC = overexpressed in SCR; green/posFC = overexpressed in PLAC1)", fig.width=8, fig.height=5, warnings=FALSE, message=FALSE}
g <- volcanoPlots(
  res.se = res.PLCvsSCR,
  string = "PLAC1vsSCR",
  first = "PLAC1",
...
```{r vol2, fig.cap="Fig X: PLAC1 vs SCR (purple/negFC = overexpressed in SCR; green/posFC = overexpressed in PLAC1)", fig.width=5, fig.height=4, warnings=FALSE, message=FALSE}
ggplotly(g)
```

### PLAC1 vs. SCR (label 2)

```{r vol3, fig.cap="Fig X: PLAC1 vs SCR (purple/negFC = overexpressed in SCR; green/posFC = overexpressed in PLAC1)", fig.width=8, fig.height=5, warnings=FALSE, message=FALSE}
g <- volcanoPlots(
  res.se = res.PLCvsSCR,
  string = "PLAC1vsSCR",
  first = "PLAC1",
...
hsPLAC1vsSCR.up.pce1 <- rat2humanID(PLAC1vsSCR.up.pce1)
hsPLAC1vsSCR.dw.pce1 <- rat2humanID(PLAC1vsSCR.dw.pce1)
hsPLAC1vsSCR.up.pce2 <- rat2humanName(PLAC1vsSCR.up.pce1)
hsPLAC1vsSCR.dw.pce2 <- rat2humanName(PLAC1vsSCR.dw.pce1)
```
...
plotPCE(inputGenes = hsPLAC1vsSCR.up.pce1, dataset = "Vt", myColor = "#00b462")
```

### Xiang

```{r pceA2, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.up.pce2, dataset = "Xi", myColor = "#00b462")
```

### Zhou&Petropoulos (Castel)

```{r pceA3, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.up.pce2, dataset = "Zp", myColor = "#00b462")
```

### Rostovskaya (Hs)

```{r pceA4, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.up.pce2, dataset = "Ro.Hs", myColor = "#00b462")
```

### Rostovskaya (Cy)

```{r pceA5, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.up.pce2, dataset = "Ro.Cy", myColor = "#00b462")
```
...
plotPCE(inputGenes = hsPLAC1vsSCR.dw.pce1, dataset = "Vt", myColor = "#980c80")
```

### Xiang

```{r pceB2, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.dw.pce2, dataset = "Xi", myColor = "#980c80")
```

### Zhou&Petropoulos (Castel)

```{r pceB3, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.dw.pce2, dataset = "Zp", myColor = "#980c80")
```

### Rostovskaya (Hs)

```{r pceB4, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.dw.pce2, dataset = "Ro.Hs", myColor = "#980c80")
```

### Rostovskaya (Cy)

```{r pceB5, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = hsPLAC1vsSCR.dw.pce2, dataset = "Ro.Cy", myColor = "#980c80")
```
...
## Genes of interest heatmaps {.tabset}

```{r pickGenes, fig.cap="", fig.width=5, fig.height=8, warnings=FALSE, message=FALSE}
geneList <- c(
  "Satb1",
  "Id1",
  "Klf5",
...
geneNames <- as.data.frame(geneList)
colnames(geneNames) <- "external_gene_name"
geneNames <- left_join(geneNames, mart, by = "external_gene_name") %>%
  dplyr::select(external_gene_name, ensembl_gene_id) %>% 
  column_to_rownames(var = "ensembl_gene_id")
dataMat <- assay(vsd)
geneNamesExp <- merge(geneNames, dataMat, by=0, all.x=TRUE)
geneNamesExp <- geneNamesExp %>% 
  dplyr::select(external_gene_name:SCR3) %>%
  column_to_rownames(var = "external_gene_name")
dataMat2 <- counts(dds, normalized = TRUE)
geneNamesExp2 <- merge(geneNames, dataMat2, by=0, all.x=TRUE)
geneNamesExp2 <- geneNamesExp2 %>% 
    dplyr::select(external_gene_name:SCR3) %>%
    column_to_rownames(var = "external_gene_name")
```
...
```{r heatmap.vst.unscaled, fig.cap="", fig.width=5, fig.height=8, warnings=FALSE, message=FALSE}
pheatmap(
    geneNamesExp,
    scale = "none",
    cluster_rows = T,
    color = rev(viridis::plasma(25)),
    cluster_cols = FALSE,
    legend = T,
)
```
...
```{r heatmap.vst.scaled, fig.cap="", fig.width=5, fig.height=8, warnings=FALSE, message=FALSE}
pheatmap(
    geneNamesExp,
    scale = "row",
    cluster_rows = T,
...
```{r heatmap.norm.unscaled, fig.cap="", fig.width=5, fig.height=8, warnings=FALSE, message=FALSE}
...
```{r heatmap.norm.scaled, fig.cap="", fig.width=5, fig.height=8, warnings=FALSE, message=FALSE}
pheatmap(
    geneNamesExp2,
    scale = "row",
    cluster_rows = T,
...
```{r keggPathway1}
pathview(gene.data=kegg_gene_list, pathway.id="rno03010", species = orgCode)
knitr::include_graphics("rno03010.pathview.png")
```
...
```{r saveInfo}
save.image(file = "assets/rTS_Diff_PLAC1_KD.RData")
save(hsPLAC1vsSCR.up.pce1, hsPLAC1vsSCR.dw.pce1, PLAC1vsSCR.up.pce1, PLAC1vsSCR.dw.pce1, file = "assets/rTS_Diff_PLAC1_KD_genelists.RData")
```
