...
ggplot(d, aes(PC1, PC2, color = condition)) +
  ...
  scale_color_manual(values = c('SCR' 		= '#002089',
                                'shPLAC1' 	= '#ff8eec')) +
  theme_bw() +
...
source("/work/LAS/geetu-lab/arnstrm/CT29_ST3D_PLAC1_KD/assets/de_functions.R")
```
...
processDE(res.SCRvsPLAC1, "SCRvsPLAC1")
```
...
```{r vol1, fig.cap="Fig X: SCR vs shPLAC1", fig.width=8, fig.height=5, warnings=FALSE, message=FALSE}
g <- volcanoPlots(
  res.SCRvsPLAC1,
  "SCRvsPLAC1",
  "SCR",
  ...
  "#4d4d4d",
  "#002089",
  ChartTitle = "SCR vs shPLAC1"
)
g
```

### SCR vs. PLAC1 (interactive)

```{r vol2, fig.cap="Fig X: SCR vs. PLAC1", fig.width=8, fig.height=5, warnings=FALSE, message=FALSE}
ggplotly(g)
```
...
```{r enrichR, warnings=TRUE, message=FALSE}
 ...
 if (websiteLive) {
   SCRvsPLAC1.up.enriched <- enrichr(SCRvsPLAC1.up.pce2, myDBs)
   SCRvsPLAC1.dw.enriched <- enrichr(SCRvsPLAC1.dw.pce2, myDBs)
 }
```
 
## PCE tests (up in SCR) {.tabset}

### VentoTormo

```{r pceA1, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.up.pce1, dataBG = teVt, cellInfo = md.vt, myColor = "#002089")
```

### Xiang

```{r pceA2, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.up.pce2, dataBG = teXi, cellInfo = md.xi, myColor = "#002089")
```

### Zhou&Petropoulos (Castel)

```{r pceA3, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.up.pce2, dataBG = teZp, cellInfo = md.zp, myColor = "#002089")
```

### Rostovskaya (Hs)

```{r pceA4, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.up.pce2, dataBG = teRo.hs, cellInfo = md.ro, myColor = "#002089")
```

### Rostovskaya (Cy)

```{r pceA5, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.up.pce2, dataBG = teRo.cy, cellInfo = md.ro, myColor = "#002089")
```

## PCE tests (up in PLAC1) {.tabset}

### VentoTormo

```{r pceB1, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.dw.pce1, dataBG = teVt, cellInfo = md.vt, myColor = "#ff8eec")
```

### Xiang

```{r pceB2, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.dw.pce2, dataBG = teXi, cellInfo = md.xi, myColor = "#ff8eec")
```

### Zhou&Petropoulos (Castel)

```{r pceB3, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.dw.pce2, dataBG = teZp, cellInfo = md.zp, myColor = "#ff8eec")
```

### Rostovskaya (Hs)

```{r pceB4, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.dw.pce2, dataBG = teRo.hs, cellInfo = md.ro, myColor = "#ff8eec")
```

### Rostovskaya (Cy)

```{r pceB5, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotPCE(inputGenes = SCRvsPLAC1.dw.pce2, dataBG = teRo.cy, cellInfo = md.ro, myColor = "#ff8eec")
```


 
## Enrichment tests (up in SCR) {.tabset}

### DisGeNET

```{r erA1, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="DisGeNET" , myColor = "#002089")
```

### GO_Biological_Process_2021

```{r erA2, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="GO_Biological_Process_2021" , "#002089")
```

### HDSigDB_Human_2021

```{r erA3, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="HDSigDB_Human_2021" , "#002089")
```

### KEGG_2021_Human

```{r erA4, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="KEGG_2021_Human" , "#002089")
```

### GTEx_Tissues_V8_2023

```{r erA5, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="GTEx_Tissues_V8_2023" , "#002089")
```

### MGI_Mammalian_Phenotype_Level_3

```{r erA6, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="MGI_Mammalian_Phenotype_Level_3" , "#002089")
```

### MGI_Mammalian_Phenotype_Level_4\_2021

```{r erA7, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="MGI_Mammalian_Phenotype_Level_4_2021" , "#002089")
```

### WikiPathways_2019_Human

```{r erA8, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="WikiPathways_2019_Human" , "#002089")
```

### Panther_2015

```{r erA9, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.up.enriched, table="Panther_2015" , "#002089")
```

## Enrichment tests (up in shPLAC1) {.tabset}

### DisGeNET

```{r erB1, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="DisGeNET" , myColor = "#ff8eec")
```

### GO_Biological_Process_2021

```{r erB2, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="GO_Biological_Process_2021" , "#ff8eec")
```

### HDSigDB_Human_2021

```{r erB3, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="HDSigDB_Human_2021" , "#ff8eec")
```

### KEGG_2021_Human

```{r erB4, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="KEGG_2021_Human" , "#ff8eec")
```

### GTEx_Tissues_V8_2023

```{r erB5, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="GTEx_Tissues_V8_2023" , "#ff8eec")
```

### MGI_Mammalian_Phenotype_Level_3

```{r erB6, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="MGI_Mammalian_Phenotype_Level_3" , "#ff8eec")
```

### MGI_Mammalian_Phenotype_Level_4\_2021

```{r erB7, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="MGI_Mammalian_Phenotype_Level_4_2021" , "#ff8eec")
```

### WikiPathways_2019_Human

```{r erB8, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="WikiPathways_2019_Human" , "#ff8eec")
```

### Panther_2015

```{r erB9, fig.cap="", fig.width=12, fig.height=5, warnings=FALSE, message=FALSE}
plotEnrichR(SCRvsPLAC1.dw.enriched, table="Panther_2015" , "#ff8eec")
```
...
save(SCRvsPLAC1.up.pce1, SCRvsPLAC1.dw.pce1, file = "assets/CT29_ST3D_PLAC1_KD_KD_genelists.RData")
```
